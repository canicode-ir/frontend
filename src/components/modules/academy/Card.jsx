  ...
  return (
    <div className="flex flex-col h-fit shadow-neomorphism p-4 rounded-2xl mt-5">
      <div className="relative flex flex-col justify-center items-center p-1">
        ...
        <Image className={`relative w-[60px] h-[60px] bg-white p-3 rounded-full z-[1] ring-2 
          ...
          src={`${imageUrl}${course.image}`} width={600} height={600} alt='course-avatar' />
        <h2 className="relative w-fit text-white text-[15px] font-demibold z-[1] mt-3 mx-2">{course.title}</h2>
        <div className={`absolute bottom-0 w-full h-[60%] ${course.name === 'htmlcss' ? 'bg-gradient-to-r from-red400 to-red600' : course.name === 'javascript' ? 'bg-gradient-to-r from-amber300 to-yellow600' : 
          course.name === 'reactjs' ? 'bg-gradient-to-r from-sky400 to-sky600' : course.name === 'nextjs' ? 'bg-gradient-to-r from-gray600 to-gray800' : course.name === 'tailwindcss' ? 'bg-gradient-to-r from-teal600 to-teal800' : 
          course.name === 'materialUi' ? 'bg-gradient-to-r from-blue600 to-blue800' : 'bg-gradient-to-r from-stone600 to-stone800'} z-0 rounded-lg`}></div>
      </div>
      <Acordion {...course}/>
      <div className="flex justify-between items-center mt-7 px-1 text-[13px]">
      ...
      <div className="flex justify-between items-center mt-3 px-1 text-[13px]">
        <div className="flex items-center">
      ...
      <div id="btn" className="flex justify-between items-center mt-5 mb-2">
        <button className="w-fit flex justify-center items-center bg-indigo600 font-bold 
          text-white text-[13px] p-2 rounded-md duration-500 hover:opacity-70">
          <PlaylistAddIcon fontSize="small" sx={{m: '0 0 0 5px'}}/>
          ثبت نام
        </button>
        <button className="w-fit flex justify-center items-center bg-indigo50 font-bold 
